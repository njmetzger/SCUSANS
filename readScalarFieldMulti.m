function [Z]=readScalarFieldMulti(a,b,ScalarFieldSelection)
...
%% Field equation from cluster robot paper
 if ScalarFieldSelection == 1
    ...
    scalarFunction1= @(x,y) M1(x,y) + M2(x,y)+M3(x,y) + R1(x,y) + R2(x,y) + T(x,y) ;
    Z = scalarFunction1(a,b);
...
elseif ScalarFieldSelection == 2
    ...
    x1_m=215;
    y1_m=150;
    scalarFunction2 = @(x,y) m1_height./(m1_rolloff.*((x-x1_m).^2+(y-y1_m).^2)+1);
    Z = scalarFunction2(a,b);
...
%     scalarFunction = @(x,y) m3_height./(m3_rolloff.*((x-x3_m).^2+(y-y3_m).^2)+1);
     ...
     error('Scalar Field Selection is not defined - check readScalarField.m')
 end
