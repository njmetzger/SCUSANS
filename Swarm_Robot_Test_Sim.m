...
[h_newsys , simName] = buildSimModel(NUM_ROBOTS,SIM_TIME, FIELD_WIDTH, SensorRange, AvoidRange, DesiredValue,CONTOUR_BUFFER,ScalarFieldSelection,x_init,y_init,radius_init);
...
%% buildSimModel()
function [h_newsys , simName] = buildSimModel(N, SIM_TIME, FIELD_WIDTH, SensorRange, AvoidRange, DesiredValue, CONTOUR_BUFFER,ScalarFieldSelection,x_init,y_init,radius_init)
...
set_param('Swarm_Robot_Base_2018a/Robot 1 Behavior/Contour Buffer', 'Value',num2str(CONTOUR_BUFFER));
set_param('Swarm_Robot_Base_2018a/Robot 1 Behavior/ScalarFieldSelection','Value',num2str(ScalarFieldSelection));
...
x_PI_ave= zeros(length(x_PI));
y_PI_ave= zeros(length(x_PI)); 

for i= 1:length(x_PI) 
    x_PI_ave(i)= mean(x_PI(i,:));
    y_PI_ave(i)= mean(y_PI(i,:));
    theta_PI(i)= mean(theta_PI(i,:)); 
    average_sensor_value(i)= readScalarField(x_PI_ave(i),y_PI_ave(i));
end 
...
switch behavior
    case 'Contour Following'
        ...
        % add the relevant legend label to the legend:
         legend_labels{1,(NUM_ROBOTS+1)}= 'Sensor Reading at Average Value';
        plot(time,average_sensor_value,'k'); % plot concentration at average of position 
        legend_labels{1,(NUM_ROBOTS+2)}= 'Desired Contour Value';
        legend_labels{1,(NUM_ROBOTS+3)}= 'Buffer Limits';
    ...
    case 'Find Min'
        ...
        legend_labels{1,(NUM_ROBOTS+1)}= 'Sensor Reading at Average Value';
        plot(time,average_sensor_value,'k'); % plot concentration at average of position 
        legend_labels{1,(NUM_ROBOTS+2)}= 'Known Global Minimum';
    ...
    case 'Find Max'
        ...
        % add reference for global max:
         legend_labels{1,(NUM_ROBOTS+1)}= 'Sensor Reading at Average Value';
        plot(time,average_sensor_value, 'k'); % plot concentration at average of position 
        legend_labels{1,(NUM_ROBOTS+2)}= 'Known Global Maximum';
